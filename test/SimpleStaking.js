...
describe("Simple Staking Test", () => {
    ...
    it("stake function", async () => {
        await stakeToken.connect(test1).approve(simpleStaking.address, ethers.utils.parseUnits("1000.0", decimals));

        // zero staking will occur error
        await expect(
            simpleStaking.connect(test1).stake(stakeToken.address, ethers.utils.parseUnits("0.0", decimals))
        ).to.be.revertedWith("Cannot stake nothing");

        await simpleStaking.connect(test1).stake(stakeToken.address, ethers.utils.parseUnits("1000.0", decimals));
        const balanceData = await stakeToken.balanceOf(test1.address);
        expect(
            ethers.utils.formatUnits(balanceData._hex, decimals).toString()
        ).to.equal("9000.0");
        
        await stakeToken.connect(test1).approve(simpleStaking.address, ethers.utils.parseUnits("1000.0", decimals));
        await expect(
            simpleStaking.connect(test1).stake(stakeToken.address, ethers.utils.parseUnits("1000.0", decimals))
        ).to.be.revertedWith("Reward rate not exist");
    });

    it("setRewardRate function", async () => {
        await expect(
            simpleStaking.connect(owner).setRewardRate(stakeToken.address, ethers.utils.parseUnits("0.0", 0))
        ).to.be.revertedWith("Reward rate should be bigger than zero");

        await simpleStaking.connect(owner).setRewardRate(stakeToken.address, ethers.utils.parseUnits("20.0", 0))
    });

    it("unstake function", async () => {
        await expect(
            simpleStaking.connect(test1).unstake(stakeToken.address, ethers.utils.parseUnits("0.0", decimals))
        ).to.be.revertedWith("Cannot unstake nothing");

        await expect(
            simpleStaking.connect(test1).unstake(test2.address, ethers.utils.parseUnits("1000.0", decimals))
        ...
        // await expect(
            await simpleStaking.connect(test1).unstake(stakeToken.address, ethers.utils.parseUnits("500.0", decimals));
    ...
    it("withdrawUnstaked function", async () => {
        await expect(
            simpleStaking.connect(test1).withdrawUnstaked(stakeToken.address, ethers.utils.parseUnits("0.0", decimals))
        ).to.be.revertedWith("Can not withdraw nothing");

        await expect(
            simpleStaking.connect(test1).withdrawUnstaked(test2.address, ethers.utils.parseUnits("1000.0", decimals))
        ).to.be.revertedWith("You dont have any unstaked to withdraw");

        await expect(
            simpleStaking.connect(test1).withdrawUnstaked(stakeToken.address, ethers.utils.parseUnits("1000.0", decimals))
        ).to.be.revertedWith("Can not withdraw over unstaked amount");
        
        await simpleStaking.connect(test1).withdrawUnstaked(stakeToken.address, ethers.utils.parseUnits("500.0", decimals));
    });
